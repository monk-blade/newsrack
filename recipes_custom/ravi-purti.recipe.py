...
class RaviPurti(BasicNewsRecipe):
    ...
    feeds = [
        ('રવિપૂર્તિ', 'https://reader.websitemachine.nl/api/query.php?user=arpanchavdaeng&t=efe94ff11b95151a57249e2cd4e79a6b&f=rss'),
    ]

    def get_cover_url(self):
        """Get the cover image URL for the current Sunday's Ravi Purti edition."""
        try:
            # Get current date
            today = date.today()
            ...
            date_str = sunday_date.strftime("%d-%m-%Y")
            
            # Construct the epaper URL for Ravi Purti
            epaper_url = f"https://epaper.gujaratsamachar.com/ravipurti/{date_str}/1"
            
            default_log.info(f"[RaviPurti] Fetching cover image from: {epaper_url}")
            ...
            # Look for pattern: src="https://epaperstatic.gujaratsamachar.com/epaper/...RAVIPURTI...-0.jpg"
            img_pattern = r'src="(https://epaperstatic\.gujaratsamachar\.com/epaper/[^"]*RAVIPURTI[^"]*-0\.jpg)"'
            match = re.search(img_pattern, content)
            
            if match:
                cover_url = match.group(1)
                default_log.info(f"[RaviPurti] Found cover image URL: {cover_url}")
                return cover_url
            else:
                default_log.warning(f"[RaviPurti] Could not find cover image in page content")
                # Fallback to masthead
                return self.masthead_url
                
        except Exception as e:
            default_log.error(f"[RaviPurti] Error fetching cover image: {str(e)}")
            # Fallback to masthead
            return self.masthead_url
    ...
    def populate_article_metadata(self, article, soup, first):
        ...
        default_log.info(f"[RaviPurti] Article: '{article.title}' | Words: {word_count} | Reading time: {minutes}m")
        default_log.info(f"[RaviPurti] Content preview: {' '.join(words[:30])}")
        ...
        if not re.search(r" \(\d+m\)$", article.title or ''):
            ...
            default_log.info(f"[RaviPurti] Updated title: {article.title}")
